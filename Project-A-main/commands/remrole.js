 ...
 */
  run: (client, message, args) => {
    if (!message.member.hasPermission(['MANAGE_MESSAGES', 'ADMINISTRATOR'])) { return message.channel.send('> *Não podes usar esse comando!*') }
    ...
    if (!message.member.roles.has(role.id)) {
      return message.reply('Não possuis esse cargo!')
    } else {
      message.member.removeRole(role)
      return message.reply(`**beep boop!** Agora não possuis mais o cargo **${role.name}**`)
    }
  },
