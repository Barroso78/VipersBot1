...
module.exports = {
  run: async (client, message, [time]) => {
    if (!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('Não tens permissão para usar esse comando!')
    ...
    if (!target.deletable) return message.reply('Eu não consigo apagar a tua ultima mensagem.')
    ...
    const sent = await message.reply(`A tua ultima mensagem será apagada em ${time} segundos.`)
